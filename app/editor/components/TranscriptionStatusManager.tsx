...
export function TranscriptionStatusManager({ documentId }: Props) {
    ...
    const replaceStatusCardWithTranscript = React.useCallback(
        (jobId: string, result: TranscriptionStatusEvent["result"]) => {
            ...
            // Create markdown content with transcript heading and code block
            const transcriptHeading = dictionary.transcript || "Transcript";
            const transcriptMarkdown = `## ${transcriptHeading}\n\n\`\`\`\n${formattedText}\n\`\`\`\n\n`;

            // Parse the markdown into ProseMirror nodes
            const transcriptContent = pasteParser.parse(
                normalizePastedMarkdown(transcriptMarkdown)
            );
            ...
            if (transcriptContent) {
                ...
                toast.success(dictionary.audioFileTranscribedSuccessfully || "Transcription completed");
            } else {
            ...
            try {
                ...
                try {
                    ...
                    if (job.status === "completed" && job.result) {
                        ...
                        Logger.info("editor", "Job already completed, replacing status card", {
                            jobId,
                        });
                        replaceStatusCardWithTranscript(jobId, job.result);
    ...
    React.useEffect(() => {
        const loadPendingJobs = async () => {
            ...
            try {
                ...
                if (view && commands.insertTranscriptionStatusCard) {
                    ...
                    for (const job of pendingJobs) {
                        if (!existingJobIds.has(job.id)) {
                            ...
                            const node = view.state.schema.nodes.transcription_status_card.create({
                                jobId: job.id,
                ...
                Logger.error("Failed to load pending transcription jobs", error as Error);
                setPendingJobsLoaded(true);
            }
        };
                ...
                for (const card of statusCards) {
                    try {
                        ...
                        if (job.status === "completed" && job.result) {
                            ...
                            replaceStatusCardWithTranscript(job.id, job.result);
                            ...
                            if (commands.updateTranscriptionStatus) {
                                commands.updateTranscriptionStatus({
                                    ...
                                    updates: {
                                        status: "failed",
                                        error: job.error || dictionary.transcriptionFailed || "Transcription failed",
                                    },
                                });
                            }
                        } else if (job.status === "cancelled") {
    ...
    }, [documentId, pendingJobsLoaded, dictionary, replaceStatusCardWithTranscript]);
