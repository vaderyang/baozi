...
export default class AutoSummaryTask extends BaseTask<Props> {
  public async perform(props: Props): Promise<void> {
    ...
    try {
      ...
      if (!job.result) {
        Logger.warn("task", "Job has no transcription result, skipping auto summary", {
          jobId,
        });
        return;
      }
      ...
      Logger.info("task", "Processing auto summary for completed transcription", {
        jobId,
      ...
      const timelineEntries = await this.generateTimelineSummaries(job.result);
   ...
   * Generate timeline summaries using the AI service.
   */
  private async generateTimelineSummaries(result: any): Promise<any[]> {
    try {
      ...
      if (!fullText.trim()) {
        Logger.warn("task", "No transcript text available for timeline summarization");
        return [];
      }

      // Use the same prompt as the frontend
   ...
   */
  private createFallbackTimeline(result: any, jobId: string): any[] {
    const segments = result.speakerSegments || [];
    const entries: any[] = [];
